...
def json_processing(json_dict, monitor_type):
    ...
    if "NN" == monitor_type:
        ...
        # FSNamesystemState
        timeStamp = float(str(json_dict['beans'][11]['BlockDeletionStartTime'])[:10])
        timeArray = time.localtime(timeStamp)
        namenode_dict['start_time'] = time.strftime('%Y-%m-%d %H:%M:%S', timeArray)

        # NameNodeInfo
        namenode_dict['hadoop_version'] = str(json_dict['beans'][3]['Version']).split(',')[0]

        return namenode_dict

    if "DFS" == monitor_type:
        # FSNamesystemState
        namenode_dict['live_nodes'] = json_dict['beans'][11]['NumLiveDataNodes']
        namenode_dict['dead_nodes'] = json_dict['beans'][11]['NumDeadDataNodes']
        namenode_dict['decommissioning_nodes'] = json_dict['beans'][11]['NumDecommissioningDataNodes']
        namenode_dict['under_replicated_blocks'] = json_dict['beans'][11]['UnderReplicatedBlocks']

        # NameNodeInfo
        namenode_dict['files_and_directorys'] = json_dict['beans'][3]['TotalFiles']
        namenode_dict['blocks'] = json_dict['beans'][3]['TotalBlocks']
        namenode_dict['configured_capacity'] = json_dict['beans'][3]['Total']
        namenode_dict['dfs_used'] = json_dict['beans'][3]['Used']
        namenode_dict['dfs_used_persent'] = min_value_display(float(json_dict['beans'][3]['PercentUsed']))
        namenode_dict['non_dfs_used'] = json_dict['beans'][3]['NonDfsUsedSpace']
        namenode_dict['dfs_remaining'] = json_dict['beans'][3]['Free']
        namenode_dict['dfs_remaining_persent'] = min_value_display(float(json_dict['beans'][3]['PercentRemaining']))
        datanodes_usages = json.loads(json_dict['beans'][3]['NodeUsage'])
        ...
        live_nodes =  json.loads(json_dict['beans'][3]['LiveNodes'])
